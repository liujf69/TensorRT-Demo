...
bool parse_args(int argc, char **argv, std::string &wts, std::string &engine, std::string &input_type, std::string &input_dir, std::string &sub_type, std::string &cuda_post_process) {
    if (argc < 4) return false;
    if (std::string(argv[1]) == "-s" && argc == 5) { // 序列化
        wts = std::string(argv[2]);
        engine = std::string(argv[3]);
        sub_type = std::string(argv[4]);
    } else if (std::string(argv[1]) == "-d" && argc == 6) { // 反序列化即推理
        engine = std::string(argv[2]);
        input_type = std::string(argv[3]); // -i:img -v:video -c:camera
        input_dir = std::string(argv[4]);
        cuda_post_process = std::string(argv[5]);
    } else {
        return false;
    }
    return true;
...
int main(int argc, char **argv) {
    ...
    // 判断输入参数是否匹配 
    if (!parse_args(argc, argv, wts_name, engine_name, input_type, input_dir, sub_type, cuda_post_process)) {
        std::cerr << "Arguments not right!" << std::endl;
        std::cerr << "./yolov8 -s [.wts] [.engine] [n/s/m/l/x]  // serialize model to plan file" << std::endl;
        std::cerr << "./yolov8 -d [.engine] [-i/-v/-c] ../samples  [c/g]// deserialize plan file and run inference" << std::endl;
        return -1;
    }
    ...
    if(input_type == "-i"){
        std::vector<std::string> file_names; // Read images
